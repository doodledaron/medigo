...
export default function NFCCheckpointScan() {
  ...
  const checkpointData = {
    '2': {
      ...
      instruction: 'Great! You\'ve reached the elevator area. Please scan to continue.',
      icon: '🛗',
      color: 'blue'
    },
    '3': {
      ...
      description: 'Hold your phone near the NFC tag at the department reception',
      instruction: 'You\'ve arrived at the Neurology Department. Please scan to check in.',
      icon: '🏥',
      color: 'green'
    }
  };

  const currentCheckpoint = checkpointData[checkpoint as keyof typeof checkpointData];
        ...
        <div className="text-center max-w-md mx-auto">
          ...
          <div className="w-32 h-32 mx-auto mb-8 relative">
            <div className={`absolute inset-0 rounded-full transition-all duration-300 ${
              isScanning 
                ? `${checkpoint === '3' ? 'bg-green-100' : 'bg-blue-100'} animate-ping` 
                : `${checkpoint === '3' ? 'bg-green-50' : 'bg-blue-50'}`
            }`}></div>
            <div className={`absolute inset-4 rounded-full transition-all duration-300 ${
              isScanning 
                ? `${checkpoint === '3' ? 'bg-green-200' : 'bg-blue-200'} animate-pulse` 
                : `${checkpoint === '3' ? 'bg-green-100' : 'bg-blue-100'}`
            }`}></div>
            <div className={`absolute inset-8 rounded-full transition-all duration-300 flex items-center justify-center ${
              isScanning 
                ? `${checkpoint === '3' ? 'bg-green-300' : 'bg-blue-300'} animate-bounce` 
          ...
          <div className="space-y-6">
            <h2 className="text-2xl font-bold text-gray-900">
              {scanComplete ? 'Scan Complete!' : isScanning ? 'Scanning...' : currentCheckpoint.subtitle}
            </h2>
